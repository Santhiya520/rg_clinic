...
<head>
    ...
    <title>Lab Test Report - {{ $manualLabTest->patient->name }}</title>
    <style>
        @page {
            size: A4;
            margin: 20mm;
...
<body>
    <div class="print-container">
        ...
        <div class="patient-info">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value"><strong>{{ $manualLabTest->patient->name }}</strong></span>
                </div>
                <div class="info-item">
                    <span class="info-label">Patient ID:</span>
                    <span class="info-value">{{ $manualLabTest->patient->patient_id }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Age / Gender:</span>
                    <span class="info-value">
                        {{ $manualLabTest->patient->age ?? 'N/A' }} yrs /
                        {{ ucfirst($manualLabTest->patient->gender ?? 'N/A') }}
                    </span>
                </div>
                <div class="info-item">
                    <span class="info-label">Reference No:</span>
                    <span class="info-value">{{ $manualLabTest->reference_no }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Phone:</span>
                    <span class="info-value">{{ $manualLabTest->patient->mobile ?? 'N/A' }}</span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">
                        <span class="test-count-badge">{{ $completedItems->count() }} Test(s)</span>
                    </span>
                </div>
                <div class="info-item">
                    <span class="info-label">Created By:</span>
                    <span class="info-value">{{ $manualLabTest->user->name ?? 'N/A' }}</span>
                </div>
            </div>
        </div>
        ...
        <!-- Loop through each completed test item -->
        @foreach($completedItems as $index => $item)
            <div class="page-break">
                ...
                <table class="test-table">
                    ...
                    <tbody>
                        @foreach($item->subTests as $subIndex => $subTest)
                        <tr>
                ...
                <div class="test-summary">
                    <strong>📊 Overall Result:</strong> {{ $item->result }}
                </div>
                @endif
                ...
                <div class="test-notes">
                    <strong>📝 Notes:</strong>
                    <p style="margin: 5px 0 0 0; color: #555;">{{ $item->notes }}</p>
                </div>
                @endif
            </div>
        @endforeach
