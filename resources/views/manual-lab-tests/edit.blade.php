...
@section('page-title', 'Edit Manual Lab Test - ' . $manualLabTest->reference_no)
    ...
    <div class="card card-preview">
        <div class="card-inner">
            <div class="preview-block">
                <form id="manualLabTestForm" action="{{ route('manual-lab-tests.update', $manualLabTest) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="form-group">
                                ...
                                <select name="patient_id" id="patient_id" class="form-control searchable-select" required>
                                    ...
                                    @foreach($patients as $patient)
                                        <option value="{{ $patient->id }}" {{ $manualLabTest->patient_id == $patient->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="payment_type" class="form-control">
                                    ...
                                    <option value="cash"
                                        {{ old('payment_type', $manualLabTest->payment_type) == 'cash' ? 'selected' : '' }}>
                                        Cash</option>
                                    <option value="card"
                                        {{ old('payment_type', $manualLabTest->payment_type) == 'card' ? 'selected' : '' }}>
                                        Card</option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div id="testItemsContainer">
                        ...
                        <div class="test-item-card card mb-3" data-index="{{ $index }}">
                            ...
                            <div class="card-body">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <select name="items[{{ $index }}][lab_test_id]"
                                                    ...
                                                    data-current-test-id="{{ $item->lab_test_id }}"
                                                    required>
                                                ...
                                                @foreach($labTests as $labTest)
                                                    <option value="{{ $labTest->id }}"
                                                        ...
                                                        {{ $item->lab_test_id == $labTest->id ? 'selected' : '' }}>
                                                        {{ $labTest->name }} - ₹{{ number_format($labTest->price, 2) }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                ...
                                <!-- Sub Tests Container -->
                                <div class="sub-tests-container mt-3" id="sub-tests-container-{{ $index }}">
                                    @php
                                        $subTestsForItem = $item->subTests;
                                    @endphp

                                    @if($subTestsForItem->isNotEmpty())
                                    <div class="card">
                                        <div class="card-header">
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <div class="form-check">
                                                    <input type="checkbox"
                                                           class="form-check-input select-all-subtests"
                                                           data-test-index="{{ $index }}"
                                                           id="select-all-{{ $index }}">
                                        ...
                                        <div class="card-body">
                                            <div class="sub-tests-list" id="sub-tests-list-{{ $index }}">
                                                @foreach($labTests->where('id', $item->lab_test_id)->first()?->subTests ?? [] as $labSubTest)
                                                    @php
                                                        $isChecked = $subTestsForItem->contains('test_name', $labSubTest->name);
                                                    @endphp
                                                    <div class="sub-test-item mb-3 p-2 border rounded">
                                                        <div class="form-check">
                                                            <input type="checkbox"
                                                                   class="form-check-input sub-test-checkbox"
                                                                   name="items[{{ $index }}][sub_tests][{{ $labSubTest->id }}][checked]"
                                                                   value="1"
                                                                   id="sub-test-{{ $index }}-{{ $labSubTest->id }}"
                                                                   data-sub-test-id="{{ $labSubTest->id }}"
                                                                   data-test-index="{{ $index }}"
                                                                   {{ $isChecked ? 'checked' : '' }}>
                                                            <label class="form-check-label fw-bold" for="sub-test-{{ $index }}-{{ $labSubTest->id }}">
                                                                {{ $labSubTest->name }}
                                                            </label>
                                                            <div class="sub-test-info mt-1 small text-muted">
                                                                <span>Unit: {{ $labSubTest->unit ?? 'N/A' }}</span> |
                                                                <span>Normal Range: {{ $labSubTest->normal_range ?? 'N/A' }}</span>
                                                            </div>
                                                            <input type="hidden" name="items[{{ $index }}][sub_tests][{{ $labSubTest->id }}][test_name]" value="{{ $labSubTest->name }}">
                                                            <input type="hidden" name="items[{{ $index }}][sub_tests][{{ $labSubTest->id }}][unit]" value="{{ $labSubTest->unit ?? '' }}">
                                                            <input type="hidden" name="items[{{ $index }}][sub_tests][{{ $labSubTest->id }}][normal_range]" value="{{ $labSubTest->normal_range ?? '' }}">
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                                    ...
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            ...
                                            <textarea name="items[{{ $index }}][notes]"
                                                      class="form-control"
                                                      rows="2"
                                                      placeholder="Enter test notes...">{{ $item->notes }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">Notes</label>
                                <textarea name="notes" class="form-control" rows="3" placeholder="Enter any notes...">{{ $manualLabTest->notes }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="border p-3 bg-light">
                                    <p><strong>Total Tests:</strong> <span id="totalTestsCount">{{ count($manualLabTest->items) }}</span></p>
                                    <p><strong>Total Amount:</strong> ₹<span id="totalAmount">{{ number_format($manualLabTest->total_amount, 2) }}</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary">
                            <em class="icon ni ni-save"></em> Update Manual Lab Test
                        </button>
...
@push('scripts')
<script>
let testItemCount = {{ count($manualLabTest->items) }};
                    ...
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="form-check">
                                <input type="checkbox"
                                       class="form-check-input select-all-subtests"
                                       data-test-index="${testItemCount}"
                                       id="select-all-${testItemCount}">
                                <label class="form-check-label" for="select-all-${testItemCount}">
                                    Select All
                                </label>
                            </div>
                        </div>
...
function removeTestItem(button) {
    ...
    cards.forEach((card, index) => {
        ...
        const selectAllCheckbox = card.querySelector('.select-all-subtests');
        if (selectAllCheckbox) {
            selectAllCheckbox.setAttribute('data-test-index', index);
            selectAllCheckbox.id = `select-all-${index}`;

            // Update label
            const label = card.querySelector(`label[for="select-all-${selectAllCheckbox.id}"]`);
            if (label) {
                label.setAttribute('for', `select-all-${index}`);
            }
        }
        ...
        // Update all sub-test checkboxes
        const subTestCheckboxes = card.querySelectorAll('.sub-test-checkbox');
        subTestCheckboxes.forEach((checkbox, subIndex) => {
            const currentName = checkbox.getAttribute('name');
            if (currentName) {
                const newName = currentName.replace(/items\[\d+\]/, `items[${index}]`);
                checkbox.setAttribute('name', newName);
            }
            checkbox.setAttribute('data-test-index', index);

            // Update associated label
            const label = card.querySelector(`label[for="${checkbox.id}"]`);
            if (label && checkbox.id) {
                const newId = checkbox.id.replace(/sub-test-\d+/, `sub-test-${index}`);
                checkbox.id = newId;
...
$(document).on('change', '.test-select', function() {
    ...
    if (subTestsData && subTestsData.length > 0) {
        ...
        // Initialize select-all functionality for this test
        initSelectAllForTest(testIndex);
    } else {
...
function initSelectAllForTest(testIndex) {
    const selectAllCheckbox = $(`#select-all-${testIndex}`);
    const subTestCheckboxes = $(`#sub-tests-container-${testIndex} .sub-test-checkbox`);

    selectAllCheckbox.off('change').on('change', function() {
        const isChecked = $(this).prop('checked');
        subTestCheckboxes.prop('checked', isChecked);
    });

    subTestCheckboxes.off('change').on('change', function() {
        const allChecked = subTestCheckboxes.length === subTestCheckboxes.filter(':checked').length;
        selectAllCheckbox.prop('checked', allChecked);
    });
}

// Initialize select-all for existing tests
function initializeAllSelectAll() {
    $('.test-item-card').each(function() {
        const testIndex = $(this).data('index');
        initSelectAllForTest(testIndex);
    });
}
...
$(document).ready(function() {
    ...
    // Initialize select-all for all existing tests
    initializeAllSelectAll();

    // Update summary on page load
    updateSummary();

    // Show sub-tests containers for existing tests if they have sub-tests
    $('.test-item-card').each(function() {
        ...
        if (subTestsData && subTestsData.length > 0) {
            $(`#sub-tests-container-${testIndex}`).show();
        }
    });
});
</script>
