...
<head>
    ...
    <title>IP Pharmacy Report - {{ \Carbon\Carbon::parse($fromDate)->format('d/m/Y') }} to {{ \Carbon\Carbon::parse($toDate)->format('d/m/Y') }}</title>
    <style>
        /* Page Setup */
        @page {
            size: A4 portrait;
...
<body>
    ...
    <div class="print-container">
        ...
        <div class="report-header">
            ...
            <h1 class="report-title">IN PATIENT PHARMACY REPORT</h1>
        ...
        <div class="summary-section no-print">
            <div class="summary-grid">
                <div class="summary-item">
                    ...
                    <div class="summary-value">{{ $ipRegisters->count() }}</div>
                </div>
                <div class="summary-item">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th width="18%" class="text-left">Patient Details</th>
                    <th width="8%">Medicines</th>
                    <th width="8%">Lab</th>
                    <th width="8%">Radiology</th>
                ...
                @forelse($ipRegisters as $index => $ip)
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <strong>{{ $ip->ip_number ?? 'N/A' }}</strong>
                        </td>
                        <td class="text-center">
                            {{ $ip->date_of_admission ? \Carbon\Carbon::parse($ip->date_of_admission)->format('d/m/Y') : 'N/A' }}
                            @if($ip->discharge_date)
                                <div class="discharge-date">
                                    → {{ \Carbon\Carbon::parse($ip->discharge_date)->format('d/m/Y') }}
                                </div>
                            @endif
                        </td>
                        <td class="text-left">
                            <div class="patient-name">{{ $ip->patient->name ?? 'N/A' }}</div>
                            <div class="patient-id">{{ $ip->patient->patient_id ?? 'N/A' }}</div>
                            @if($ip->patient->mobile)
                                <div class="patient-id">{{ $ip->patient->mobile }}</div>
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="12" class="text-center" style="padding: 30px;">
                            <div class="no-data">
                                ...
                                <div class="no-data-text">No IP records found for the selected period</div>
                            </div>
                        </td>
                    </tr>
                @endforelse
