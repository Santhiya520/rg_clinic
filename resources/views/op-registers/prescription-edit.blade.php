...
@section('content')
    <div class="nk-block nk-block-lg">
        <form action="{{ route('op-registers.prescription.update', ['opRegister' => $opRegister->id]) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="nk-block-head">
                        <div class="nk-block-between">
                            <div class="nk-block-head-content">
                                ...
                                <p class="text-soft">Prescription ID: {{ $opRegister->id }}</p>
                            </div>
                            <div class="nk-block-head-content">
                                <span
                                    class="badge bg-{{ $opRegister->status == 'active' ? 'success' : ($opRegister->status == 'completed' ? 'primary' : 'warning') }}">
                        ...
                        <div class="nk-block-between">
                            <div class="nk-block-head-content">
                                <p class="text-soft">Patient ID:
                                    <strong>{{ $opRegister->patient?->patient_id ?? 'N/A' }}</strong></p>
                            </div>
                            <div class="nk-block-head-content">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="provisional_diagnosis" rows="3" required>{{ old('provisional_diagnosis', $opRegister->provisional_diagnosis) }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">Investigations</label>
                                <textarea class="form-control" name="investigations" rows="3">{{ old('investigations', $opRegister->investigations) }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">Final Diagnosis</label>
                                <textarea class="form-control" name="final_diagnosis" rows="3">{{ old('final_diagnosis', $opRegister->final_diagnosis) }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">Treatment</label>
                                <textarea class="form-control" name="treatment" rows="3">{{ old('treatment', $opRegister->treatment) }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="result">
                                    ...
                                    <option value="cured"
                                        {{ old('result', $opRegister->result) == 'cured' ? 'selected' : '' }}>Cured
                                    </option>
                                    <option value="improved"
                                        {{ old('result', $opRegister->result) == 'improved' ? 'selected' : '' }}>Improved
                                    </option>
                                    <option value="not_improved"
                                        {{ old('result', $opRegister->result) == 'not_improved' ? 'selected' : '' }}>Not
                                        Improved</option>
                                    <option value="referred"
                                        {{ old('result', $opRegister->result) == 'referred' ? 'selected' : '' }}>Referred
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="status" required>
                                    <option value="active"
                                        {{ old('status', $opRegister->status) == 'active' ? 'selected' : '' }}>Active
                                    </option>
                                    <option value="completed"
                                        {{ old('status', $opRegister->status) == 'completed' ? 'selected' : '' }}>Completed
                                    </option>
                                    <option value="cancelled"
                                        {{ old('status', $opRegister->status) == 'cancelled' ? 'selected' : '' }}>Cancelled
                                    </option>
                                </select>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="additional_information" rows="2">{{ old('additional_information', $opRegister->additional_information) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="d-flex flex-wrap gap-3 mb-2">
                                                ...
                                                <div class="form-check">
                                                    <input type="checkbox"
                                                        class="form-check-input timing-checkbox sos-checkbox"
                                                        name="medicines[{{ $index }}][sos]" value="1"
                                                        id="sos-{{ $index }}"
                                                        {{ $medicine->sos ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="sos-{{ $index }}">SOS</label>
                                                </div>
                                                <div class="form-check">
                                                    <input type="checkbox"
                                                        class="form-check-input timing-checkbox ml-checkbox"
                                                        name="medicines[{{ $index }}][ml]" value="1"
                                                        id="ml-{{ $index }}" {{ $medicine->ml ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="ml-{{ $index }}">ML</label>
                                                </div>
                                            </div>

                                            <!-- Injection Routes -->
                                            <div class="injection-routes {{ str_contains(strtolower($medicine->medicine->category ?? ''), 'inject') || str_contains(strtolower($medicine->medicine->category ?? ''), 'injection') ? 'active' : '' }}"
                                                id="injection-routes-{{ $index }}">
                                                <label class="form-label d-block mb-2">Injection Route</label>
                                                <div class="d-flex flex-wrap gap-3">
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input route-checkbox"
                                                            name="medicines[{{ $index }}][im_route]"
                                                            value="1" id="im-route-{{ $index }}"
                                                            {{ $medicine->im_route ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="im-route-{{ $index }}">IM</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input route-checkbox"
                                                            name="medicines[{{ $index }}][iv_route]"
                                                            value="1" id="iv-route-{{ $index }}"
                                                            {{ $medicine->iv_route ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="iv-route-{{ $index }}">IV</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input route-checkbox"
                                                            name="medicines[{{ $index }}][id_route]"
                                                            value="1" id="id-route-{{ $index }}"
                                                            {{ $medicine->id_route ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="id-route-{{ $index }}">ID</label>
                                                    </div>
                                                    <div class="form-check">
                                                        <input type="checkbox" class="form-check-input route-checkbox"
                                                            name="medicines[{{ $index }}][sub_q_route]"
                                                            value="1" id="sub-q-route-{{ $index }}"
                                                            {{ $medicine->sub_q_route ? 'checked' : '' }}>
                                                        <label class="form-check-label"
                                                            for="sub-q-route-{{ $index }}">SUB-Q</label>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control days-input"
                                                ...
                                                value="{{ $medicine->no_of_days }}" min="0">
                                        </div>
                                    </div>
                                    <div class="col-md-1">
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control quantity-input"
                                                ...
                                                value="{{ $medicine->quantity }}" min="0">
                                        </div>
                                    </div>

                                    <input type="number" class="form-control price-input"
                                        name="medicines[{{ $index }}][price]" value="{{ $medicine->price }}"
                                        step="0.01" required hidden>
            ...
            <div class="card card-preview mt-3">
                <div class="card-inner">
                    ...
                    <div id="radiology-container">
                        @foreach ($opRegister->radiologyTests as $index => $radiologys)
                            <div class="radiology-row border p-3 mb-3">
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control radiology-select select2-search"
                                                ...
                                                @foreach ($radiology as $test)
                                                    <option value="{{ $test->id }}"
                                                        {{ $radiologys->radiology_test_id == $test->id ? 'selected' : '' }}
                                    ...
                                    <input type="text" class="form-control radiology-price"
                                        name="radiologies[{{ $index }}][price]"
                                        value="{{ $radiologys->radiologyTest->price ?? '' }}" hidden>

                                    <!-- Notes -->
                                    <div class="col-md-5">
                                        <div class="form-group">
                                            <label class="form-label">Notes</label>
                                            <textarea class="form-control" name="radiologies[{{ $index }}][notes]" rows="2">{{ $radiologys->notes }}</textarea>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="card-inner">
                    ...
                    <div id="lab-tests-container">
                        @foreach ($opRegister->labTests as $index => $labTest)
                            @php
                                $selectedSubTests = $labTest->subTests->pluck('lab_sub_test_id')->toArray();
                            @endphp
                            <div class="lab-test-row border p-3 mb-3" data-lab-index="{{ $index }}">
                                <div class="row">
                                    <!-- Lab Test Select -->
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control labtest-select select2-search"
                                                ...
                                                @foreach ($labTests as $test)
                                                    <option value="{{ $test->id }}"
                                                        ...
                                                        data-price="{{ $test->price }}"
                                                        data-sub-tests="{{ json_encode($test->subTests) }}">
                                                        {{ $test->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <input type="text" class="form-control labtest-price"
                                        ...
                                        value="{{ $labTest->labTest->price ?? '' }}" hidden>
                                ...
                                <!-- Sub Tests Container -->
                                <div class="sub-tests-container" id="sub-tests-{{ $index }}" style="display: {{ $labTest->labTest->subTests->count() > 0 ? 'block' : 'none' }};">
                                    <h6 class="mb-3">Select Sub Tests</h6>
                                    <div class="sub-tests-list" id="sub-tests-list-{{ $index }}">
                                        <!-- Sub tests will be loaded dynamically via JavaScript -->
                                    </div>
                                </div>
                    ...
                    <button type="button" class="btn btn-sm btn-primary" id="add-lab-test">
...
@push('scripts')
    <script>
        ...
        let radiologyIndex = {{ $opRegister->radiologyTests->count() }};
        let labTestIndex = {{ $opRegister->labTests->count() }};

        // Store selected sub tests for each lab test
        let selectedSubTests = {};

        @foreach ($opRegister->labTests as $index => $labTest)
            selectedSubTests[{{ $index }}] = {{ json_encode($labTest->subTests->pluck('lab_sub_test_id')->toArray()) }};
        @endforeach
        ...
        <div class="medicine-row border p-3 mb-3">
            <div class="row">
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        <label class="form-label">No. of Days</label>
                        <input type="number" class="form-control days-input" name="medicines[${medicineIndex}][no_of_days]" min="0">
                    </div>
                </div>
                <div class="col-md-1">
                    <div class="form-group">
                        <label class="form-label">Qty</label>
                        <input type="number" class="form-control quantity-input" name="medicines[${medicineIndex}][quantity]" min="0">
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="form-label">Lab Test *</label>
                        <select class="form-control labtest-select select2-search"
                            name="lab_tests[${labTestIndex}][lab_test_id]" required
                            data-lab-index="${labTestIndex}">
                            <option value="">Select Lab Test</option>
                            @foreach ($labTests as $test)
    ...
    `;
        ...
        $(document).on('change', '.labtest-select', function() {
            ...
            if (subTestsData && subTestsData.length > 0) {
                ...
                // Get previously selected sub tests for this lab test
                const existingSelections = selectedSubTests[labIndex] || [];

                subTestsData.forEach(function(subTest, idx) {
                    const isChecked = existingSelections.includes(subTest.id);
                    ...
                    const subTestHtml = `
                        <div class="sub-test-item">
                            <div class="form-check">
                                ...
                                <input type="hidden" name="lab_tests[${labIndex}][sub_tests][${subTest.id}][order]" value="${subTest.order || idx}">
                            </div>
                        </div>
                    `;
        ...
        $(document).on('change', '.sub-test-checkbox', function() {
            ...
            // Update selectedSubTests object
            if (!selectedSubTests[labIndex]) {
                selectedSubTests[labIndex] = [];
            }

            if (isChecked) {
                if (!selectedSubTests[labIndex].includes(subTestId)) {
                    selectedSubTests[labIndex].push(subTestId);
                }
            } else {
                const index = selectedSubTests[labIndex].indexOf(subTestId);
                if (index > -1) {
                    selectedSubTests[labIndex].splice(index, 1);
                }
            }
        });
        ...
        $(document).on('click', '.remove-lab-test', function() {
            const row = $(this).closest('.lab-test-row');
            const labIndex = row.data('lab-index');

            // Clean up selectedSubTests for this lab test
            delete selectedSubTests[labIndex];
        ...
        $(document).ready(function() {
            // Initialize medicine category display for existing records
            ...
            // Load sub tests for existing lab tests
            $('.labtest-select').each(function() {
                const labTestRow = $(this).closest('.lab-test-row');
                const labIndex = $(this).data('lab-index');
                const selectedOption = $(this).find('option:selected');
                const subTestsData = selectedOption.data('sub-tests');
                ...
                if (subTestsData && subTestsData.length > 0) {
                    ...
                    // Get existing selections for this lab test
                    const existingSelections = selectedSubTests[labIndex] || [];
