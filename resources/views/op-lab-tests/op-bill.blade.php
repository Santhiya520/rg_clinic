...
<head>
    ...
    <title>Lab Test Report - {{ $opLabTest->labTest->name }}</title>
    <style>
...
<body>
    <div class="print-container">
        ...
        <div class="patient-info">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $opLabTest->opRegister->patient->name }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Patient ID:</span>
                    <span class="info-value">{{ $opLabTest->opRegister->patient->patient_id }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Age / Gender:</span>
                    <span class="info-value">
                        {{ $opLabTest->opRegister->patient->age ?? 'N/A' }} years /
                        {{ ucfirst($opLabTest->opRegister->patient->gender ?? 'N/A') }}
                    </span>
                </div>
                <div class="info-item">
                    <span class="info-label">Token No:</span>
                    <span class="info-value">{{ $opLabTest->opRegister->token_number }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Test Name:</span>
                    <span class="info-value"><strong>{{ $opLabTest->labTest->name }}</strong></span>
                </div>
                <div class="info-item">
        ...
        <div style="margin-top: 20px; padding: 15px; border: 1px solid #dee2e6; background: #f8f9fa;">
            ...
            <p style="margin: 0; color: #555;">{{ $opLabTest->result }}</p>
        </div>
        @endif
        ...
        <div style="margin-top: 15px; padding: 12px; border-left: 3px solid #3498db; background: #e8f4fd;">
            ...
            <p style="margin: 0; color: #555;">{{ $opLabTest->notes }}</p>
        </div>
        @endif

        <!-- Status Information -->
        <div style="margin-top: 15px; padding: 10px; text-align: center;">
            <span style="padding: 5px 15px; background: {{ $opLabTest->status == 'completed' ? '#28a745' : ($opLabTest->status == 'pending' ? '#ffc107' : '#dc3545') }}; color: white; border-radius: 20px;">
                Status: {{ strtoupper($opLabTest->status) }}
            </span>
            @if($opLabTest->completed_at)
            <span style="margin-left: 20px; color: #666;">
                Completed on: {{ $opLabTest->completed_at->format('d/m/Y h:i A') }}
            </span>
            @endif
        </div>
<!-- Signature Area -->
