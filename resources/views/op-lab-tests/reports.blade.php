    ...
    <div class="card card-preview">
        <div class="card-inner">
            ...
            <div class="card-inner border-bottom no-print">
                <form action="{{ route('lab.reports') }}" method="GET" class="form-validate" id="reportForm">
                    @csrf
                    <div class="row g-3">
                                <select name="patient_type" class="form-control" required id="patientType" hidden>
                                    <option value="all" {{ $patientType == 'all' ? 'selected' : '' }}>All Patients</option>
                                    <option value="op" {{ $patientType == 'op' ? 'selected' : '' }}>OP Patients</option>
                                    <option value="ip" {{ $patientType == 'ip' ? 'selected' : '' }}>IP Patients</option>
                                    <option value="manual" {{ $patientType == 'manual' ? 'selected' : '' }}>Manual Tests</option>
                                </select>
                        ...
                        <div class="col-md-12">
                            <div class="form-group mt-2">
                                ...
                                <a href="{{ route('lab.reports') }}" class="btn btn-secondary">
            ...
            <div class="card-inner" id="resultsSection">
                @if($searchPerformed)
                    @if($totalRecords > 0)
                        <!-- OP Patients Results -->
                        @if(($patientType === 'all' || $patientType === 'op') && $opResults->count() > 0)
                            <div class="mb-5">
                                <h6 class="title mb-3 border-bottom pb-2">
                                    <i class="fas fa-user-md text-primary mr-2"></i>
                                    OP Patients Lab Tests ({{ $opResults->total() }})
                                </h6>
                                        ...
                                        <tbody>
                                            @foreach($opResults as $result)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="no-print">
                                                        <a href="{{ route('lab.op.show', $result) }}"
                        ...
                        <!-- IP Patients Results -->
                        @if(($patientType === 'all' || $patientType === 'ip') && $ipResults->count() > 0)
                            <div class="mb-5">
                                <h6 class="title mb-3 border-bottom pb-2">
                                    <i class="fas fa-procedures text-info mr-2"></i>
                                    IP Patients Lab Tests ({{ $ipResults->total() }})
                                </h6>
                                        ...
                                        <tbody>
                                            @foreach($ipResults as $result)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>₹{{ number_format($totalAmount, 2) }}</td>
                                                    <td class="no-print">
                                                        <a href="{{ route('lab.ip.show', $result) }}"
                                                           class="btn btn-sm btn-primary" title="View">
                                                            <em class="icon ni ni-eye"></em>
                                                        </a>
                                                    </td>
                        ...
                        @if(($patientType === 'all' || $patientType === 'manual') && $manualResults->count() > 0)
                            <div class="mb-5">
                                <h6 class="title mb-3 border-bottom pb-2">
                                    <i class="fas fa-file-medical text-success mr-2"></i>
                                    Manual Lab Tests ({{ $manualResults->total() }})
                                </h6>
                                        ...
                                        <tbody>
                                            @foreach($manualResults as $result)
                                                <tr>
                                                    ...
                                                    <td class="no-print">
                                                        <a href="{{ route('manual-lab-tests.show', $result) }}"
                                                           class="btn btn-sm btn-primary" title="View">
                                                            <em class="icon ni ni-eye"></em>
                                                        </a>
                                                    </td>
                    ...
                    @else
                        ...
                        <div class="text-center py-5">
                            ...
                            <div class="mt-3">
                                <a href="{{ route('lab.reports') }}" class="btn btn-primary">
                ...
                @else
                    ...
                    <div class="text-center py-5">
                        <em class="icon ni ni-search text-muted" style="font-size: 3rem;"></em>
                        <h5 class="mt-3">Search Lab Reports</h5>
                        <p class="text-muted">Please use the filters above and click "Search" to find lab reports</p>
                    </div>
                @endif
            </div>
        </div>
