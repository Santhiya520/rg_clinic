...
<head>
    ...
    <title>IP Lab Test Report - {{ $ipLabTest->labTest->name }}</title>
    <style>
...
<body>
    <div class="print-container">
        ...
        <div class="patient-info">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $ipLabTest->inpatientRegister->patient->name }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Patient ID:</span>
                    <span class="info-value">{{ $ipLabTest->inpatientRegister->patient->patient_id }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Age / Gender:</span>
                    <span class="info-value">
                        {{ $ipLabTest->inpatientRegister->patient->age ?? 'N/A' }} years /
                        {{ ucfirst($ipLabTest->inpatientRegister->patient->gender ?? 'N/A') }}
                    </span>
                </div>
                <div class="info-item">
                    <span class="info-label">IP No:</span>
                    <span class="info-value">{{ $ipLabTest->inpatientRegister->hospital_ip_no }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Admission Date:</span>
                    <span class="info-value">{{ $ipLabTest->inpatientRegister->date_of_admission->format('d/m/Y') }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Test Name:</span>
                    <span class="info-value"><strong>{{ $ipLabTest->labTest->name }}</strong></span>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $ipLabTest->inpatientRegister->room_no ?? 'N/A' }}</span>
                </div>
            </div>
        </div>

        <!-- Test Results Table -->
        @if($ipLabTest->subTests->isNotEmpty())
        ...
        <table class="test-table">
            ...
            <tbody>
                @foreach($ipLabTest->subTests as $index => $subTest)
                <tr>
        ...
        <div style="margin-top: 20px; padding: 15px; border: 1px solid #dee2e6; background: #f8f9fa;">
            ...
            <p style="margin: 0; color: #555;">{{ $ipLabTest->result }}</p>
        </div>
        @endif
        ...
        <div style="margin-top: 15px; padding: 12px; border-left: 3px solid #3498db; background: #e8f4fd;">
            ...
            <p style="margin: 0; color: #555;">{{ $ipLabTest->notes }}</p>
        </div>
        @endif

        <!-- Status Information -->
        <div style="margin-top: 15px; padding: 10px; text-align: center;">
            <span style="padding: 5px 15px; background: {{ $ipLabTest->status == 'completed' ? '#28a745' : ($ipLabTest->status == 'pending' ? '#ffc107' : '#dc3545') }}; color: white; border-radius: 20px;">
                Status: {{ strtoupper($ipLabTest->status) }}
            </span>
            @if($ipLabTest->completed_at)
            <span style="margin-left: 20px; color: #666;">
                Completed on: {{ $ipLabTest->completed_at->format('d/m/Y h:i A') }}
            </span>
            @endif
        </div>

        <!-- Doctor Information -->
        @if($ipLabTest->inpatientRegister->medicalOfficer)
        <div style="margin-top: 15px; padding: 10px; border: 1px solid #dee2e6;">
            <h4 style="margin-bottom: 5px; color: #2c3e50; font-size: 13px;">Attending Doctor</h4>
            <p style="margin: 0; color: #555;">
                Dr. {{ $ipLabTest->inpatientRegister->medicalOfficer->name }}
                @if($ipLabTest->inpatientRegister->medicalOfficer->qualification)
                    ({{ $ipLabTest->inpatientRegister->medicalOfficer->qualification }})
                @endif
            </p>
        </div>
        @endif
        ...
        <!-- Footer -->
        <div class="footer">
            <p>*** INPATIENT LABORATORY TEST REPORT ***</p>
        ...
        <div class="no-print">
            ...
            <p style="margin-top: 10px; color: #666; font-size: 11px;">
                Press Ctrl+P or click the button above to print this report
            </p>
        </div>
    </div>
